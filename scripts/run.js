...
const APIv2 = "https://api.twitter.com/2";
const APIv1 = "https://api.twitter.com/1.1";
...
function buildUrl(api, pathname, params) {
...
async function resolveUserId(username){
  const d = await xGet(APIv2, `/users/by/username/${encodeURIComponent(username)}`, { "user.fields":"id,username" });
  return d?.data?.id;
}

async function fetchNewestSince(userId, sinceId){
  const params={ exclude:"retweets,replies", "max_results":"5", "tweet.fields":"created_at" };
  if(sinceId) params.since_id=sinceId;
  const d = await xGet(APIv2, `/users/${userId}/tweets`, params);
  const arr = d?.data ?? [];
  return arr.length ? [arr[0]] : [];
}

async function getMyUserId(){
  const me = await xGet(APIv1, "/account/verify_credentials.json", { skip_status:"true", include_entities:"false" });
  return me?.id_str;
}

async function retweet(actorUserId,tweetId){
  try{ await xPost(APIv2, `/users/${actorUserId}/retweets`, { tweet_id: tweetId }); return true;}
  catch(e){const s=String(e); if(s.includes("403")||s.includes("429")) return false; throw e;}
}

async function reply(text,inReplyToId){
  return xPost(APIv2, `/tweets`, { text, reply:{ in_reply_to_tweet_id: inReplyToId } });
}
...
async function main(){
  ...
  if(!myId){ console.error("Could not resolve my user id"); process.exit(1); }
  ...
  if(!tweets.length){ console.log("No new tweets"); return; }
